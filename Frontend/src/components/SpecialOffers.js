...
const SpecialOffers = ({ role }) => {
  const { isDark, toggleTheme } = useTheme(); // Get theme from context
  ...
  return (
    <div className={`flex min-h-screen ${isDark ? 'dark' : 'bg-gray-100'}`}>
      ...
      <div className="flex-1 ml-64 p-10">
        <motion.div
          className={`shadow-xl rounded-lg p-8 ${isDark ? 'bg-gray-800' : 'bg-white'}`}
        ...
        >
          ...
          >
            {offers.map((offer) => (
              <motion.li
                key={offer.id}
                className={`mb-6 p-4 rounded-lg shadow-lg ${isDark ? 'bg-gradient-to-r from-indigo-600 via-purple-600 to-blue-600' : 'bg-gradient-to-r from-indigo-500 via-purple-500 to-blue-500'}`}
              ...
              >
                ...
                <p className={`text-lg ${isDark ? 'text-gray-300' : 'text-gray-200'}`}>
