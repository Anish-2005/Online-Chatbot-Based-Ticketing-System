  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-gray-950 text-white' : 'bg-white text-gray-900'}`}>
      <div className="relative overflow-hidden">
        <div className={`pointer-events-none absolute inset-0 ${isDark ? 'bg-gradient-to-br from-gray-950 via-purple-950/15 to-gray-950' : 'bg-gradient-to-br from-white via-purple-50/40 to-white'}`} />
          ...
          >
            <div className={`relative flex-1 min-w-[240px] max-w-md`}>
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-2.5 rounded-xl text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-purple-500/40 ${isDark ? 'bg-gray-800 border-gray-700/50 text-white placeholder-gray-500' : 'bg-gray-100 border-gray-200 text-gray-900 placeholder-gray-400'} border`}
              />
            </div>
            ...
            <div className={`flex rounded-xl border overflow-hidden ${isDark ? 'border-gray-700/50' : 'border-gray-200'}`}>
              <button
                ...
                className={`p-2.5 transition-all ${viewMode === 'grid' ? (isDark ? 'bg-purple-600 text-white' : 'bg-purple-600 text-white') : (isDark ? 'bg-gray-800 text-gray-400 hover:text-white' : 'bg-white text-gray-500 hover:text-gray-900')}`}
              >
                <FiGrid className="h-4 w-4" />
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`p-2.5 transition-all ${viewMode === 'list' ? (isDark ? 'bg-purple-600 text-white' : 'bg-purple-600 text-white') : (isDark ? 'bg-gray-800 text-gray-400 hover:text-white' : 'bg-white text-gray-500 hover:text-gray-900')}`}
              >
                <FiList className="h-4 w-4" />
              </button>
            </div>

            <span className={`rounded-full px-3 py-1.5 text-xs font-bold ${isDark ? 'bg-purple-900/40 text-purple-300' : 'bg-purple-100 text-purple-700'}`}>
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
        ...
        {!loading && error && (
          ...
          >
            ...
            <button
              onClick={loadShows}
              className="mt-3 px-4 py-2 rounded-xl text-sm font-bold bg-gradient-to-r from-purple-600 to-pink-600 text-white"
            >
              Try Again
            </button>
          </motion.div>
          ...
          <div className={`grid gap-4 ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
            {filteredShows.map((show, index) => (
              <motion.div
                ...
                className={`group cursor-pointer rounded-2xl border overflow-hidden transition-all duration-300 ${isDark ? 'border-gray-700/50 bg-gray-800/60 hover:border-purple-500/40 hover:shadow-lg hover:shadow-purple-500/5' : 'border-gray-200 bg-white hover:border-purple-300 hover:shadow-xl hover:shadow-purple-500/10'}`}
              >
                {/* Image */}
                <div className="relative overflow-hidden">
                  <img
                  ...
                  <div className="absolute top-3 right-3">
                    <span className="rounded-full bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm px-3 py-1.5 text-sm font-bold text-purple-700 dark:text-purple-300 shadow-lg">
                      {show.price || '₹100'}
                    </span>
                  </div>
                ...
                {/* Content */}
                <div className="p-5">
                  <h3 className={`text-lg font-heading font-bold mb-2 transition-colors group-hover:text-purple-600 dark:group-hover:text-purple-400 ${isDark ? 'text-white' : 'text-gray-900'}`}>
