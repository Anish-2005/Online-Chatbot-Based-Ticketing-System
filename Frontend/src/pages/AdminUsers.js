...
const AdminUsers = () => {
    ...
    return (
        <div className="flex">
            <Sidebar role="admin" />
            <motion.div
                className={`flex-1 p-8 ml-64 ${isDarkMode ? 'dark-mode' : 'text-black'}`}
                initial={{ opacity: 0, x: 50 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.5, ease: 'easeOut' }}
            >
                <div className="flex justify-between items-center mb-6">
                    <h1 className={`text-3xl font-bold mb-6 ${isDarkMode ? 'text-violet-400' : 'text-black'}`}>
                        User Management
                    </h1>
                    <button
                        onClick={toggleDarkMode}
                        className={`py-2 px-4 rounded-lg font-semibold shadow-md transition-all duration-300 ease-in-out ${isDarkMode
                ...
                <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8 overflow-x-auto">
                        ...
                        <tbody>
                            {users_info.map(user => (
                                <motion.tr
                                    ...
                                    className="hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200 dark:text-gray-200"
                                >
